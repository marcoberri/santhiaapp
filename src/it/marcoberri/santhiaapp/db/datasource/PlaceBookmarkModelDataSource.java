...
public class PlaceBookmarkModelDataSource {
	...
	values.put(PlaceBookmarkModelDBEntry.COLUMN_NAME_TS, new SimpleDateFormat(FORMAT_DATE).format(new Date()));
	...
	final SQLiteDatabase db = helper.getWritableDatabase();
	final Cursor c = db.query(PlaceBookmarkModelDBEntry.TABLE_NAME, ALLFIELD, PlaceBookmarkModelDBEntry.COLUMN_NAME_PLACE_ID + " = ?", new String[] { placeId.toString() }, null, null, PlaceBookmarkModelDBEntry.COLUMN_NAME_TS);

	final PlaceBookmarkModel bookmark = new PlaceBookmarkModel();

	if (c.moveToFirst()) {

	    bookmark.setPlaceId(c.getInt(c.getColumnIndex(PlaceBookmarkModelDBEntry.COLUMN_NAME_PLACE_ID)));
	    try {
		bookmark.setTs(new SimpleDateFormat(FORMAT_DATE).parse(c.getString(c.getColumnIndex(PlaceBookmarkModelDBEntry.COLUMN_NAME_TS))));
	...
	final SQLiteDatabase db = helper.getWritableDatabase();
	int tot_delete = db.delete(PlaceBookmarkModelDBEntry.TABLE_NAME, PlaceBookmarkModelDBEntry.COLUMN_NAME_PLACE_ID + " = ?", new String[] { placeId.toString() });
