...
public class PlaceGpsModelDataSource {
    ...
    public static final String[] ALLFIELDS = new String[] { PlaceGpsModelDBEntry.COLUMN_NAME_PLACE_ID, PlaceGpsModelDBEntry.COLUMN_NAME_LAT, PlaceGpsModelDBEntry.COLUMN_NAME_LNG };
	...
	final SQLiteDatabase db = helper.getWritableDatabase();
	final Cursor c = db.query(PlaceGpsModelDBEntry.TABLE_NAME, ALLFIELDS, PlaceGpsModelDBEntry.COLUMN_NAME_PLACE_ID + " = ?", new String[] { placeId.toString() }, null, null, null);

	final PlaceGpsModel gps = new PlaceGpsModel();

	if (c.moveToFirst()) {
	    gps.setPlace_id(c.getInt(c.getColumnIndex(PlaceGpsModelDBEntry.COLUMN_NAME_PLACE_ID)));
	    gps.setLat((c.getFloat(c.getColumnIndex(PlaceGpsModelDBEntry.COLUMN_NAME_LAT))));
	    gps.setLng((c.getFloat(c.getColumnIndex(PlaceGpsModelDBEntry.COLUMN_NAME_LNG))));
	}
