...
public class FragmentPlaceDetail extends Fragment {
	...
	final TextView title = (TextView) view.findViewById(R.id.place_detail_title);
	title.setText(placeModel.getTitle());

	final TextView text = (TextView) view.findViewById(R.id.place_detail_text);
	...
	final PlaceDetailGalleryPageAdapter galleryPageAdapter = new PlaceDetailGalleryPageAdapter(context.getSupportFragmentManager(), fragments);
	final ViewPager pager = (ViewPager) view.findViewById(R.id.place_detail_gallery);

	pager.removeAllViewsInLayout();
	pager.setAdapter(galleryPageAdapter);
	pager.setPageTransformer(true, new ZoomOutPageTransformer());
	mIndicator = (CirclePageIndicator) view.findViewById(R.id.place_detail_gallery_indicator);
	mIndicator.setViewPager(pager);

	galleryPageAdapter.notifyDataSetChanged();

	map = ((MapFragment) getFragmentManager().findFragmentById(R.id.tab_3_map)).getMap();

	if (map != null) {

	    final LatLng gpsPos = new LatLng(placeModel.getGps().getLat(), placeModel.getGps().getLng());

	    Marker kiel = map.addMarker(new MarkerOptions().position(gpsPos).title(placeModel.getTitle()).snippet(placeModel.getSubtitle() + "\n" + placeModel.getAddress())
	...
	super.onDestroyView();
	final MapFragment f = (MapFragment) getFragmentManager().findFragmentById(R.id.tab_3_map);
	if (f != null)
	    getFragmentManager().beginTransaction().remove(f).commit();

	final MapFragment f2 = (MapFragment) getFragmentManager().findFragmentById(R.id.tab_2);
	if (f2 != null)
	    getFragmentManager().beginTransaction().remove(f2).commit();

	final MapFragment f1 = (MapFragment) getFragmentManager().findFragmentById(R.id.tab_1);
	if (f1 != null)
	    getFragmentManager().beginTransaction().remove(f1).commit();

	final MapFragment f4 = (MapFragment) getFragmentManager().findFragmentById(R.id.tab_4);
	if (f4 != null)
	...
	tabSpec1.setIndicator(getResources().getString(R.string.tab_story));
	...
	tabSpec3.setIndicator(getResources().getString(R.string.tab_map));

	final TabSpec tabSpec4 = mTabHost.newTabSpec("Comments");
	tabSpec4.setContent(R.id.tab_4);
	tabSpec4.setIndicator(getResources().getString(R.string.tab_comment));
	...
	final PlaceBookmarkModel bookmark = dsBookMark.getPlaceBookmarkByPlaceId(placeModel.getId());
